...
class LapModel:
    def __init__(self, path=DATA / "race_model.json"):
        ...
        self.tyres = {t: Tyre(m["tyres"][t]["offset_s"], m["tyres"][t]["wear_s_per_lap"], m["tyres"][t]["life_laps"]) for t in ALL_TYRES}
        self.dry_in_rain = m["rain"]["dry_tyre_s_at_full_rain"]
        self.wet_when_dry = m["rain"]["wet_tyre_s_when_dry"]
        self.wet_in_rain = m["rain"]["wet_tyre_s_at_full_rain"]
    ...
    def rain_penalty(self, tyre, rain):
        """Seconds lost for the weather: rain 0 is dry, 1 is the heaviest rain. Wet tyres win above about 0.23."""
        if tyre == "WET":
            return self.wet_when_dry * (1 - rain) + self.wet_in_rain * rain
        return self.dry_in_rain * rain

    def lap_time(self, tyre, age, lap, wear=1.0, pace=0.0, rain=0.0):
        """Predicted time for lap number `lap` on a tyre that is `age` laps old (counting this lap)."""
        t = self.tyres[tyre]
        worn_laps = age * wear
        past_life = max(0.0, worn_laps - t.life)
        return (self.base + pace + t.offset + t.wear * worn_laps + self.fuel * lap
                + WORN_OUT_S_PER_LAP * past_life + self.rain_penalty(tyre, rain))

    def fastest_lap(self, lap, pace=0.0, rain=0.0, tyres=TYRES):
        """Lower bound for any lap: best of `tyres`, no wear. Used by the A* heuristic."""
        return self.base + pace + self.fuel * lap + min(self.tyres[t].offset + self.rain_penalty(t, rain) for t in tyres)
